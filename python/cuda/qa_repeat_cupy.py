...
@pytest.mark.parametrize("interp", [1, 3, 17, 1000])
def test_matches_cpu(dtype, vlen, interp):
    num_items = 257
    values = np.arange(num_items * vlen).astype(dtype)
    if np.issubdtype(dtype, np.complexfloating):
        values += 1j * values[::-1]

    expected, _ = run_repeat(make_cpu_repeat, values, interp, vlen, dtype)
    result, _ = run_repeat(
        make_gpu_repeat,
        values,
        interp,
...
def test_tags_match_cpu():
    ...
    assert [tag.offset for tag in result_tags] == [tag.offset for tag in expected_tags]
    assert [pmt.to_long(tag.value) for tag in result_tags] == [
        pmt.to_long(tag.value) for tag in expected_tags
    ]


def test_partial_repetitions_across_work_calls():
    interp = 7
    vlen = 3
...
def test_output_multiple_and_runtime_interpolation():
    repeat = cuda.repeat_cupy(interp=7, output_multiple=16)
    assert repeat.output_multiple() == 16
    assert repeat.interpolation() == 7

    repeat.set_interpolation(11)
    assert repeat.interpolation() == 11
    ...
    try:
        while repeat.interpolation() != 13 and time.monotonic() < deadline:
            time.sleep(0.01)
        assert repeat.interpolation() == 13
    finally:
        tb.stop()
        tb.wait()
...
def test_invalid_runtime_interpolation():
