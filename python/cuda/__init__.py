...
def as_cupy(numpy_array):
    ...
    # Note: pointerGetAttributes throws an error for invalid/host pointers in some versions,
    # or returns attributes with type=0 (host) in others.
    try:
        attrs = cp.cuda.runtime.pointerGetAttributes(ptr)
        # attrs.type: 0=Unregistered Host, 1=Host, 2=Device, 3=Managed
        # Note: cupy attribute is named 'type', not 'memoryType'
