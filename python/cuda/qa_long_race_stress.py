...
class wrap_check_sink(gr.sync_block):
    ...
    def __init__(self, expected):
        gr.sync_block.__init__(
            self, "wrap_check_sink", in_sig=[np.complex64], out_sig=[]
        )
    ...
    def work(self, input_items, output_items):
        ...
        if not np.array_equal(in_data, expected_slice):
            ...
            print(
                "Sequence mismatch (possible buffer sync issue), count: ",
                self.mismatch_count,
                "not mismatch count: ",
                self.notmismatch_count,
            )
...
class qa_long_race_stress(gr_unittest.TestCase):
    ...
    @unittest.skipUnless(
        os.environ.get("GR_CUDA_LONG_STRESS") == "1",
    ...
    def test_001_fanin_fanout_chain_long(self):
        ...
        x = (
            np.arange(nitems, dtype=np.float32)
            + 1j * np.zeros(nitems, dtype=np.float32)
        ).astype(np.complex64)
        ...
        branches = []
        for _branch_idx in range(fanout):
            chain = []
        ...
        if total_mismatches:
            for i, c in enumerate(checkers):
                if c.mismatch_count:
                    print(
                        f"Checker {i}: mismatches={c.mismatch_count},"
                        f" ok={c.notmismatch_count}"
                    )
        self.assertEqual(
            total_mismatches, 0, f"Total sequence mismatches: {total_mismatches}"
        )
