...
class qa_fft(gr_unittest.TestCase):
    ...
    def _make_input(self, fft_size, real_input):
        ...
        return (
            self.rng.standard_normal(total) + 1j * self.rng.standard_normal(total)
        ).astype(np.complex64)
    ...
    def _compare_fft(self, runner, fft_size, forward, window, shift, real_input):
        ...
        expected = _run_native_fft(data, fft_size, forward, window, shift, real_input)
        actual = runner(data, fft_size, forward, window, shift, real_input)
        ...
        for fft_size in self.fft_sizes:
            for real_input in (False, True):
                with self.subTest(fft_size=fft_size, real_input=real_input):
                    self._compare_fft(
                        _run_cuda_fft, fft_size, True, [], False, real_input
                    )
    ...
    def test_cuda_fft_sizes_inverse(self):
        for fft_size in self.fft_sizes:
            for real_input in (False, True):
                with self.subTest(fft_size=fft_size, real_input=real_input):
                    self._compare_fft(
                        _run_cuda_fft, fft_size, False, [], False, real_input
                    )
    ...
    def test_cuda_fft_window(self):
        ...
        for real_input in (False, True):
            with self.subTest(real_input=real_input):
                self._compare_fft(
                    _run_cuda_fft, fft_size, True, window, False, real_input
                )
    ...
    def test_cupy_fft_sizes_forward(self):
        for fft_size in self.fft_sizes:
            for real_input in (False, True):
                with self.subTest(fft_size=fft_size, real_input=real_input):
                    self._compare_fft(
                        _run_cupy_fft, fft_size, True, [], False, real_input
                    )
    ...
    def test_cupy_fft_sizes_inverse(self):
        for fft_size in self.fft_sizes:
            for real_input in (False, True):
                with self.subTest(fft_size=fft_size, real_input=real_input):
                    self._compare_fft(
                        _run_cupy_fft, fft_size, False, [], False, real_input
                    )
    ...
    def test_cupy_fft_window(self):
        ...
        for real_input in (False, True):
            with self.subTest(real_input=real_input):
                self._compare_fft(
                    _run_cupy_fft, fft_size, True, window, False, real_input
                )
