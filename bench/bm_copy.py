...
class benchmark_copy(gr.top_block):
    def __init__(self, args):
        ...
        for _i in range(num_blocks):
            copy_blocks.append(
                cuda.load(self.load, gr.sizeof_gr_complex * veclen, not self.no_cb)
            )

        self.blocks_null_source_0 = blocks.null_source(gr.sizeof_gr_complex * veclen)
        self.blocks_null_sink_0 = blocks.null_sink(gr.sizeof_gr_complex * veclen)
        self.blocks_head_0 = blocks.head(gr.sizeof_gr_complex * veclen, actual_samples)
        ...
        self.connect((copy_blocks[num_blocks - 1], 0), (self.blocks_null_sink_0, 0))
