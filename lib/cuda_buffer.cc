 ...
 *
 * By default events use spin-polling on cudaEventSynchronize, which
 * yields the lowest latency but burns a CPU core per waiting thread.
 * To switch to blocking sync (thread sleeps), set in the path returned
